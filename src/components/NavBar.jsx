...
function NavBar() {
  return (
    <NavBarDiv>
      <LogoDiv> <StledNavLink  style={({ isActive }) => {
          return isActive ? { opacity: "0.8" } : { opacity: "1" };
        }}  to="/"><StyledImg src="src\assets\logo.png" alt="Logo for Ryan's blog website"/></StledNavLink></LogoDiv>
      <RightLinksDiv>
      <StledNavLink  style={({ isActive }) => {
          return isActive ? { color: "Gray" } : { color: "black" };
        }} to="/about">about</StledNavLink>
      <StledNavLink to="/contact"  style={({ isActive }) => {
          return isActive ? { color: "Gray" } : { color: "black" };
        }}>contact</StledNavLink>
      </RightLinksDiv>
    </NavBarDiv>
  );
}
